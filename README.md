...
The separate [before screenshot](before-mock.png) uses the repository's **mock-Gateway rendered UI** regression scenario with the old catch restored (`setChatError(host, rejected ? formatUiError(error) : null)`). The test clicked the real rendered **Check outcome** button, injected `UNAVAILABLE`, asserted `.chat-error` remained absent, and captured the screen; **1/1 selected case passed** in 17.86 s (1.241 s case). The source/test changes were temporary and the PR worktree was restored clean. This is a comparison of the exact old UI branch, **not** a real-Gateway before trace or a side-by-side identical fixture. A clean-source real-Gateway counterfactual was attempted but its session-change refresh removed the recovery banner before the follow-up click; those failed setups are not represented as proof.
